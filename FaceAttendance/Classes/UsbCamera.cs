...
namespace FaceAttendance.Classes
{
    class UsbCamera
    {
        ...
        private Action<IntPtr, System.Drawing.Size> SetPreviewControlMain;

        /// <summary>Set preview on control. Call before starts.</summary>
        /// <param name="handle">control handle.</param>
        public void SetPreviewControl(IntPtr handle, System.Drawing.Size size) { SetPreviewControlMain(handle, size); }

        /// <summary>Set preview size.</summary>
        public void SetPreviewSize(System.Drawing.Size size) { SetPreviewControlMain(IntPtr.Zero, size); }
        ...
        /// </param>
        public UsbCamera(int cameraIndex, System.Drawing.Size size) : this(cameraIndex, new VideoFormat() { Size = size })
        {
        }
        ...
        private void Init(int index, VideoFormat format)
        {
            ...
            //SetResolution(vcap_source, 500, 500);
            ...
            //    // Calculate the position to center the 500x500 video within the window
            //    var x = (clientSize.Width - videoWidth) / 2;
            //    var y = (clientSize.Height - videoHeight) / 2;
            //    xt = x;
            //    yt = y;
            //    vw.SetWindowPosition((int)x, (int)y, videoWidth, videoHeight);
            //};
        ...
        private class SampleGrabberCallback : DirectShow.ISampleGrabberCB
        {
            ...
            GetBitmap(int width, int height, int stride)
            {
                ...
                lock (BufferLock)
                {
                    ...
                    //return BufferToBitmap(Buffer, 500, 500, stride);
                }
            }
        ...
        private static void SetVideoOutputFormat(DirectShow.IPin pin, VideoFormat format)
        {
            ...
            {
                var item = formats[0];
                //new System.Drawing.Size(480, 640);
                for (int j = 0; j < formats.Length; j++)
                {
                    Guid FORMAT_VideoInfo2 = new Guid("{F72A76A0-EB0A-11d0-ACE4-0000C0CC16BA}");
                    if (formats[j].Size.Width == 640 && formats[j].Size.Height == 480 && formats[j].Caps.Guid == FORMAT_VideoInfo2)
                    {
                        SetVideoOutputFormat(pin, i, formats[j].Size, formats[j].TimePerFrame);
                        return;

                    }
                }
            //    SetVideoOutputFormat(pin, i, formats[17].Size, formats[17].TimePerFrame);
            //return;
            ...
            SetVideoOutputFormat(pin, 0, System.Drawing.Size.Empty, 0);
        }
        ...
        private static VideoFormat[] GetVideoOutputFormat(DirectShow.IPin pin)
        {
            ...
            for (int i = 0; i < cap_count; i++)
            {
                ...
                if (mt.FormatType == DirectShow.DsGuid.FORMAT_VideoInfo)
                {
                    ...
                    entry.Size = new System.Drawing.Size(vinfo.bmiHeader.biWidth, vinfo.bmiHeader.biHeight);
        ...
        private static void SetVideoOutputFormat(DirectShow.IPin pin, int index, System.Drawing.Size size, long timePerFrame)
        {
            // IAMStreamConfigインタフェース取得
            var config = pin as DirectShow.IAMStreamConfig;
            if (config == null)
        ...
        public class VideoFormat
        {
            ...
            public System.Drawing.Size Size { get; set; }         // ビデオサイズ
    ...
    static class DirectShow
    {
        ...
        public interface IAMVideoControl
        {
            ...
            int GetMaxAvailableFrameRate([In] IPin pPin, [In] int iIndex, [In] System.Drawing.Size Dimensions, [Out] out long MaxAvailableFrameRate);
            int GetFrameRateList([In] IPin pPin, [In] int iIndex, [In] System.Drawing.Size Dimensions, [Out] out int ListSize, [Out] out IntPtr FrameRates);
        }
